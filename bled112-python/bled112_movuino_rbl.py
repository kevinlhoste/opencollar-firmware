...
def my_ble_evt_attclient_find_information_found(sender, args):
    ...
    elif args['uuid'] == list(reversed(uuid_client_characteristic_configuration)) and attr_mv_rx_handle > 0 and attr_mv_rx_handle_ccc == 0:
...
def my_ble_evt_attclient_procedure_completed(sender, args):
    ...
    if state == STATE_FINDING_ATTRIBUTES:
        if attr_mv_rx_handle > 0 and attr_mv_tx_handle > 0 and attr_mv_rx_handle_ccc > 0:
            ...
            ble.send_command(ser, ble.ble_cmd_attclient_attribute_write(connection_handle, attr_mv_rx_handle_ccc, [0x01, 0x00]))
            ble.check_activity(ser, 1)
            state = STATE_WRITING
        else:
